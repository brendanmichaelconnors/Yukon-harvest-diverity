...
# source functions
source("1_Yuk_harv_div_functions.R")
...
ScenC_spwn <- readRDS(file = "outputs/ScenarioC_RR_spawners.RDS")
ScenD_spwn <- readRDS(file = "outputs/ScenarioD_RR_spawners.RDS")
ScenE_spwn <- readRDS(file = "outputs/ScenarioE_RR_spawners.RDS")

ScenCDE_spwn <- data.frame(cbind(ScenC_spwn,ScenD_spwn,ScenE_spwn))
ScenCDE_spwn$CDEmean <- rowMeans(ScenCDE_spwn)
...
df <- data.frame(year,pop,ScenCDE_spwn$CDEmean, ScenB_spwn)
...
df2 <- df %>%
  mutate(Population = case_when(pop == "LowerMainstem" ~ "Lower Mainstem",
                                pop == "Stewart" ~ "Stewart",
                                pop == "Pelly" ~ "Pelly",
                                pop == "WhiteDonjek" ~ "White-Donjek",
                                pop == "MiddleMainstem" ~ "Middle Mainstem",
                                pop == "Carmacks" ~ "Carmacks",
                                pop == "UpperLakesAndMainstem" ~ "Upper Lakes and Mainstem",
                                pop == "Teslin" ~ "Teslin"))


df2$Population <- factor(df2$Population, levels = c("Lower Mainstem","Stewart","Pelly", "White-Donjek",
                                    "Middle Mainstem","Carmacks","Upper Lakes and Mainstem","Teslin"))

ggplot(df2, aes(x=ScenCDE_spwn.CDEmean/1000, y=ScenB_spwn/1000,color=year)) +
  geom_point(size=2.5) +
...
#ggsave(file.path(figure_path, paste("Figure_S1.spawner.compare_",system_time,".jpg", sep="")),height = 9, width = 7)
...
ggsave(file.path(figure_path, paste("Figure_3.border.count_",system_time,".jpg", sep="")),height = 9, width = 9)
```
...
```{r  Prep Population border passage over time for Run Reconstruction, echo=FALSE}
...
```{r Figure 8 - border passage by population over time, echo=FALSE, fig.dim=c(5,7)}
...
ScenA_data <- ScenA_data %>%
  mutate(Color = case_when(stock == "LowerMainstem" ~ "#440154FF",
                           stock == "Stewart" ~ "#46337EFF",
                           stock == "Pelly" ~ "#365C8DFF",
                           stock == "WhiteDonjek" ~ "#277F8EFF",
                           stock == "MiddleMainstem" ~ "#1FA187FF",
                           stock == "Carmacks" ~ "#4AC16DFF",
                           stock == "UpperLakesAndMainstem" ~ "#9FDA3AFF",
                           stock == "Teslin" ~ "#FDE725FF")) %>%
...
ScenA_data$Population <- factor(ScenA_data$Population, levels = c("Lower Mainstem","Stewart","Pelly","White-Donjek","Middle Mainstem",
                                                                              "Carmacks","Upper Lakes and Mainstem","Teslin"))

ggplot(ScenA_data, aes(x = as.factor(year), y = MLE/1000, fill= Population)) + 
  ...
  geom_errorbar(data=ScenA_data, aes(ymin = MLE/1000-SE/1000, ymax = MLE/1000+SE/1000), width=0.1) +
...
# ggsave(file.path(figure_path, paste("Figure_8.popn.passage_ScenA_",system_time,".jpg", sep="")),height = 9, width = 7)
```


```{r Plot Scenario B population run proportions Figure 8, echo=FALSE }
# 

ScenB_data$population <- factor(ScenB_data$population, levels = c("Lower Mainstem","Stewart","Pelly","White-Donjek","Middle Mainstem",
                                                                              "Carmacks","Upper Lakes and Mainstem","Teslin"))


ScenB_data <- ScenB_data %>%
  mutate(Color = case_when(population == "LowerMainstem" ~ "#440154FF",
                           population == "Stewart" ~ "#46337EFF",
                           population == "Pelly" ~ "#365C8DFF",
                           population == "WhiteDonjek" ~ "#277F8EFF",
                           population == "MiddleMainstem" ~ "#1FA187FF",
                           population == "Carmacks" ~ "#4AC16DFF",
                           population == "UpperLakesandMainstem" ~ "#9FDA3AFF",
                           population == "Teslin" ~ "#FDE725FF"))


ggplot(ScenB_data, aes(x = as.factor(year), y = proportion*100, fill = population)) + 
  geom_bar(stat = "identity",colour = "black", width=1) +
  scale_fill_manual(values = c("#440154FF", "#46337EFF", "#365C8DFF","#277F8EFF", "#1FA187FF", "#4AC16DFF","#9FDA3AFF", "#FDE725FF")) +
  ...
  theme(axis.text = element_text(size=10)) +
  theme(axis.title = element_text(size=10))
#ggsave(file.path(figure_path, paste("Figure_8.popn.passage_ScenB_",system_time,".jpg", sep="")),height = 9, width = 7)
...
PostC <- read.csv("Posterior_Samples_C.csv")
PostD <- read.csv("Posterior_Samples_D.csv")
PostE <- read.csv("Posterior_Samples_E.csv")

Post_comb <- rbind(PostC, PostD, PostE)
...
```{r productivity dataframe Scenario A for Figure 12, echo=FALSE}
...
```{r productivity dataframe Scenario B for Figure 12, echo=FALSE}
...
```{r Productivity for Figure 12 dataframe Post_comb (posteriors C, D, E combined dataframe), echo=FALSE}

# Using Post_comb (posteriors C, D, E combined dataframe)
year <- rep(c(1982:2016),each=24)

alpha_comb <- Post_comb %>%
  select(1:8) %>%
  gather(key = alpha, value = alpha_value, alpha_1:alpha_8) %>%
  mutate(population = rep(c("Lower Mainstem", "Stewart", "Pelly","White-Donjek","Middle Mainstem", "Carmacks",
                          "Upper Lakes and Mainstem", "Teslin"),each=30000)) %>%
  group_by(population) %>%
  summarise(alpha = list(enframe(quantile(alpha_value, probs=c(0.25,0.5,0.75))))) %>%
  unnest() %>%
  ungroup() %>%
  mutate(log_alpha = log(value)) %>%
  slice(rep(row_number(),35)) %>%
  mutate(year = year) %>%
  rename(log_alpha_value = value) %>%
  select(population, name, year, log_alpha) %>%
  spread(key = name, value = log_alpha) %>%
  rename(log_alpha_lwr = "25%",
         log_alpha_med = "50%",
         log_alpha_upr = "75%") %>%
  as.data.frame()
...
# residuals dataframe
year <- rep(c(1982:2016),each=30000)

resid_comb <- Post_comb %>%
  select(634:913) %>%
  gather(key = resid, value = resid_value, log_resid_1_1:log_resid_35_8) %>%
  mutate(year = rep(year, times=8),
         population = rep(c("Lower Mainstem", "Stewart", "Pelly","White-Donjek","Middle Mainstem", "Carmacks",
                          "Upper Lakes and Mainstem", "Teslin"),each=1050000)) %>%
  group_by(population, year) %>%
  summarise(resid = list(enframe(quantile(resid_value, probs=c(0.25,0.5,0.75))))) %>%
  unnest() %>%
  ungroup() %>%
  rename(log_resid = value) %>%
  select(population, name, year, log_resid) %>%
  spread(key = name, value = log_resid) %>%
  rename(log_resid_lwr = "25%",
         log_resid_med = "50%",
         log_resid_upr = "75%") %>%
  as.data.frame()
  
alpha_resid_df_comb <- full_join(alpha_comb,resid_comb)
...
```{r alpha and beta posteriors ScenarioA for Figure 10, echo=FALSE}
...
```{r alpha and beta posteriors ScenarioB for Figure 10, echo=FALSE}
# alpha and beta posteriors
...
```{r alpha and beta posteriors combined Scenario C/D/E for Figure 10, echo=FALSE}
# alpha and beta posteriors
alpha.beta_comb <- Post_comb %>%
  as.data.frame() %>%
  select(1:16)
...
# get percentiles
alpha_summ <- rbind(quantile(ScenarioB_posteriors$alpha_1, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_2, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_3, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_4, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_5, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_6, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_7, c(0.05,0.5,0.95)),
quantile(alpha.beta_comb$alpha_8, c(0.05,0.5,0.95)))

alpha_summary_comb <- as.data.frame(cbind(pop, alpha_summ)) %>%
  rename(alpha_low = "5%",
         alpha_med = "50%",
         alpha_upr = "95%")

alpha_summary_comb$alpha_low <- as.numeric(as.character(alpha_summary_comb$alpha_low))
alpha_summary_comb$alpha_med <- as.numeric(as.character(alpha_summary_comb$alpha_med))
alpha_summary_comb$alpha_upr <- as.numeric(as.character(alpha_summary_comb$alpha_upr))

# create equilibrium values, = log(alpha)/beta
equil_product_comb <- Post_comb[,1:16] %>%
  gather(key = alpha_pop, value = alpha_value, alpha_1:alpha_8) %>%
  gather(key = beta_pop, value = beta_value, beta_1:beta_8) %>%
  mutate(equilibrium = log(alpha_value)/beta_value,
         population = substr(beta_pop, 6,6))
...
equil_product_comb_perc <- equil_product_comb %>%
  group_by(population) %>%
  summarise(equil_lwr = quantile(equilibrium, probs = 0.05),
            equil_med = quantile(equilibrium, probs = 0.5),
            equil_upr = quantile(equilibrium, probs = 0.95)) %>%
...
equil_alpha_percentilecomb <- cbind(equil_product_comb_perc,alpha_summary_comb) %>%
  select(-population, -region)
...
```{r Figure 10 ScenA and ScenB productivity vs. equilibrium size, echo=FALSE, fig.dim=c(6,6)}

# dataframe used is equil_alpha_percentileA
# head(equil_alpha_percentileA)

# Scenario A
equil_alpha_percentileA$pop <- factor(equil_alpha_percentileA$pop, levels = c("Lower Mainstem","Stewart","Pelly",
...
#ggsave(file.path(figure_path, paste("Figure_10.ScenA.product.equilibrium_",system_time,".jpg", sep="")),height = 5, width = 7)




# Scenario B
equil_alpha_percentileB$pop <- factor(equil_alpha_percentileB$pop, levels = c("Lower Mainstem","Stewart","Pelly",
                                                                              "White-Donjek","Middle Mainstem",
                                                                              "Carmacks","Upper Lakes and Mainstem","Teslin"))
...
ggplot(data = equil_alpha_percentileB, aes(x = equil_med/1000, y = alpha_med, fill=pop)) +
  ...
  theme(axis.text = element_text(size=12)) +
  theme(legend.title=element_blank())
#ggsave(file.path(figure_path, paste("Figure_10.ScenB.product.equilibrium_",system_time,".jpg", sep="")),height = 5, width = 7)
...
# Scenario combined C/D/E
equil_alpha_percentilecomb$pop <- factor(equil_alpha_percentilecomb$pop, levels = c("Lower Mainstem","Stewart","Pelly",
                                                                              "White-Donjek","Middle Mainstem",
                                                                              "Carmacks","Upper Lakes and Mainstem","Teslin"))

ggplot(data = equil_alpha_percentilecomb, aes(x = equil_med/1000, y = alpha_med, fill=pop)) +
  geom_errorbar(data=equil_alpha_percentilecomb, mapping=aes(x=equil_med/1000, ymin=alpha_low, ymax=alpha_upr), width=0,color = "gray50") +
  geom_errorbarh(data=equil_alpha_percentilecomb, mapping=aes(x=alpha_med, xmin=equil_lwr/1000, xmax=equil_upr/1000), height=0,color = "gray50") +
  geom_point(size=4, pch=21, colour="black") +
  xlab("Equilibrium size (000s)") +
  ylab("Intrinsic productivity") +
  scale_fill_viridis(option = "viridis", discrete = TRUE) +
...
#ggsave(file.path(figure_path, paste("Figure_10.ScencombCDE.product.equilibrium_",system_time,".jpg", sep="")),height = 5, width = 7)
...
```{r recruits and spawners dataframe Scenario combined C/D/E, echo=FALSE}
...
R_50 <- stack(lapply(Recruits_Scencomb, quantile, prob = 0.5, names = FALSE)) %>%
  mutate(year = rep(c(1:35),times=8),
         ...
         param = "recruits",
         percentile = "median")
R_05 <- stack(lapply(Recruits_Scencomb, quantile, prob = 0.05, names = FALSE)) %>%
  mutate(year = rep(c(1:35),times=8),
         population = rep(pop, each = 35),
         param = "recruits",
         percentile = "lower")
R_95 <- stack(lapply(Recruits_Scencomb, quantile, prob = 0.95, names = FALSE)) %>%
  mutate(year = rep(c(1:35),times=8),
         population = rep(pop, each = 35),
         param = "recruits",
         percentile = "upper")

Recruits_Scencomb_df <- rbind(R_50, R_05, R_95) %>%
  mutate(adj_year = as.numeric(as.character(year)) - 7) %>%
  filter(adj_year > 0) %>%
  select(4,3,7,6,5,1) %>%
  spread(key = percentile, value = values) %>%
...
S_50 <- stack(lapply(Spawners_Scencomb, quantile, prob = 0.5, names = FALSE)) %>%
  mutate(year = rep(c(1:32),times=8),
         ...
         param = "spawners",
         percentile = "median")
S_05 <- stack(lapply(Spawners_Scencomb, quantile, prob = 0.05, names = FALSE)) %>%
  mutate(year = rep(c(1:32),times=8),
         population = rep(pop, each = 32),
         param = "spawners",
         percentile = "lower")
S_95 <- stack(lapply(Spawners_Scencomb, quantile, prob = 0.95, names = FALSE)) %>%
  mutate(year = rep(c(1:32),times=8),
         population = rep(pop, each = 32),
         param = "spawners",
         percentile = "upper")

Spawners_Scencomb_df <- rbind(S_50, S_05, S_95) %>%
  mutate(adj_year = as.numeric(as.character(year))) %>%
  select(4,3,7,6,5,1) %>%
  spread(key = percentile, value = values) %>%
  rename(Spw_low = lower,
...
```{r Spawner recruit relationships Scenario combined C/D/E, echo=FALSE}
...
```{r Figure 11 Plot Ricker curves ScenarioA, echo=FALSE, warning=FALSE,message=FALSE, fig.dim=c(6,6)}
...
```{r Figure 11 Plot Ricker curves ScenarioB, echo=FALSE, warning=FALSE,message=FALSE, fig.dim=c(6,6)}
...
```{r Figure 11 Plot Ricker curves Scenario combined C/D/E, echo=FALSE, warning=FALSE,message=FALSE, fig.dim=c(6,6)}
...
ggplot() +
  geom_ribbon(data = alpha_beta_percentilecomb, aes(x = abund/1000, ymin = q_05/1000, ymax = q_95/1000), 
              fill = "grey80", alpha=0.5, linetype=2, colour="gray46") +
  geom_line(data = alpha_beta_percentilecomb, aes(x = abund/1000, y = q_50/1000), color="black", size = 1.25) +
...
```{r Figure 12. Productivity index over time - Scenario A, fig.dim=c(8,8)}
...
```{r Figure 12. Productivity index over time - Scenario B, fig.dim=c(8,8)}

# head(alpha_resid_df_B)

alpha_resid_df_B$population <- factor(alpha_resid_df_B$population, levels = c("Lower Mainstem","Stewart","Pelly",
                                                                              "White-Donjek","Middle Mainstem",
                                                                              "Carmacks","Upper Lakes and Mainstem","Teslin"))

ggplot(alpha_resid_df_B, aes(x=year, y = alpha_med + resid_med, color=population), show.legend = F) +
  geom_point(size=1,show.legend = F) + 
  scale_color_manual(values = viridis(8), name=population) +
  geom_line(show.legend = F) + 
  geom_ribbon(aes(ymin=alpha_lwr + resid_lwr, ymax=alpha_upr + resid_upr), show.legend = F, alpha=0.4) +
  scale_color_manual(values = viridis(8)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust = 1, size=10))
#ggsave(file.path(figure_path, paste("Figure_12.Prod.index_ScenB_",system_time,".jpg", sep="")),height = 9, width = 7)

```


```{r Figure 12. Productivity index over time - Scenario comb C/D/E, fig.dim=c(8,8)}
...
```{r Umsy dataframe ScenarioA and ScenarioB for Figure 13, echo=FALSE}
...
```{r Figure 13. Usmy posteriors Scenario A, echo=FALSE, fig.dim=c(6,6)}
...
```{r Figure 13. Usmy posteriors Scenario B, echo=FALSE, fig.dim=c(6,6)}
# Scenario B
...
ggplot(Umsy_B, aes(x=substock, y=value, fill=substock)) +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(legend.position="none")
#ggsave(file.path(figure_path, paste("Figure_13.Umsy_ScenB_",system_time,".jpg", sep="")),height = 5, width = 7)
...
```{r Figure 13. Usmy posteriors Scenario comb C/D/E, echo=FALSE, fig.dim=c(6,6)}
# Scenario C/D/E
...
ggplot(Umsy_comb, aes(x=substock, y=value*100, fill=substock)) +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(legend.position="none")
#ggsave(file.path(figure_path, paste("Figure_12.Umsy_Scen.comb_",system_time,".jpg", sep="")),height = 5, width = 7)
...
alpha <- alpha.beta_comb %>%
  select(1:8) %>%
  gather(key = alpha, value = alpha_value, alpha_1:alpha_8) %>%
  mutate(population = rep(pop,each=30000)) %>%
  select(-alpha)
...
```{r Figure 13 tradeoff plots Scenario comb C/D/E, echo=FALSE, message=FALSE,warning=FALSE, fig.dim=c(6,6)}
# head(t3.3)

# panel a: overfished
ggplot(t3.3, aes(x=U*100, y=med_V2/900)) +
  ggtitle("Overfished") +
  geom_line(linetype = "solid", size=1.25, color="blue") +
  geom_ribbon(aes(ymin=low_V2/900, ymax=upp_V2/900), alpha=0.2, color="blue", lty="dashed", fill = "blue") +
  geom_line(aes(x=U*100,y=over.med*120), color="red", size=1.25) +
  geom_ribbon(aes(ymin=over.low*120, ymax=over.up*120), alpha=0.2, color="red", lty="dashed", fill= "red") +
  scale_y_continuous(limits = c(0,120), breaks=c(0,20,40,60,80,100,120)) +
  scale_y_continuous(sec.axis = sec_axis(~./1.2, name = "Populations overfished (%)")) +
  ...
  annotate("text", x=5, y=110,label="A", size=8)
ggsave(file.path(figure_path, paste("Figure_13.tradeoff.overfish_ScencombCDE_",system_time,".jpg", sep="")),height = 5, width = 7)
```



```{r Extirpation Scenario comb C/D/E Figure 13 tradeoffs, echo=FALSE,message=FALSE,warning=FALSE, fig.dim=c(6,6)}
...
ggplot(t3.3, aes(x=U*100, y=med_V2/900)) +
  ...
  geom_line(linetype = "solid", size=1.25, color="blue") +
  geom_ribbon(aes(ymin=low_V2/900, ymax=upp_V2/900), alpha=0.2, color="blue", lty="dashed", fill = "blue") +
  geom_line(aes(x=U*100,y=ext.med*120), color="red", size=1.25) +
  geom_ribbon(aes(ymin=ext.low*120, ymax=ext.up*120), alpha=0.2, color="red", lty="dashed", fill= "red") +
  scale_y_continuous(limits = c(0,120), breaks=c(0,20,40,60,80,100,120)) +
  scale_y_continuous(sec.axis = sec_axis(~./1.2, name = "Populations exirpated (%)")) +
  scale_x_continuous(limits = c(0,100), breaks=c(0,20,40,60,80,100)) +
  ylab("Harvest (000s)") +
  xlab("Harvest rate (%)") +
  theme_bw() +
  ...
  theme(axis.title.x = element_text(size=12)) +
  theme(plot.title = element_text(hjust = 0.5, size=12)) +
  annotate("text", x=5, y=110,label="B", size=8)
ggsave(file.path(figure_path, paste("Figure_13.tradeoff.extirpate_ScencombCDE_",system_time,".jpg", sep="")),height = 5, width = 7)
...
```{r Portfolio effects calculations, echo=FALSE}
...
```{r Figure 14 Portfolio Effects, echo=FALSE}

jpeg(file.path(figure_path, paste("Figure_12.portfolio_effects_",system_time,".jpg",sep="")),width=7.75, height=3.5, units="in",res=800)

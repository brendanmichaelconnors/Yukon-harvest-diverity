...
View(df)
ggplot(df %>% filter(year > "1983"), aes(x=julian_date, y=diff)) +
  geom_bar(stat="identity") +
  facet_wrap(~year)
...
ggplot() +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(axis.text = element_text(size=10))
# ggsave(file.path(figure_path, paste("Figure_9.Ricker_ScenA_",system_time,".pdf", sep="")),height = 5, width = 7)
...
ggplot() +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(axis.text = element_text(size=10))
# ggsave(file.path(figure_path, paste("Figure_9.Ricker_ScenB",system_time,".pdf", sep="")),height = 5, width = 7)
...
ggplot(alpha_resid_df_B, aes(x=year, y = alpha_med + resid_med), show.legend = F) + #, fill=population
  geom_point(size=1,show.legend = F) + #, aes(colour = factor(population))
  geom_line(show.legend = F) + #, aes(colour = factor(population))
  geom_ribbon(aes(ymin=alpha_lwr + resid_lwr, ymax=alpha_upr + resid_upr), show.legend = F, alpha=0.4) + # , colour=factor(population)
  scale_fill_viridis(option = "viridis", discrete = TRUE) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust = 1, size=10))
#ggsave(file.path(figure_path, paste("Figure_10.Prod.index_ScenB_",system_time,".pdf", sep="")),height = 5, width = 7)
...
```{r Extirpation ScenA Figure 13 tradeoffs, echo=FALSE}

...
df$pop <- factor(df$pop, levels = c("LowerMainstem","Stewart","Pelly", "WhiteDonjek",
                                    "MiddleMainstem","Carmacks","UpperLakesAndMainstem","Teslin"))

ggplot(df, aes(x=ScenA_spwn/1000, y=ScenB_spwn/1000,color=year)) +
  geom_point(size=2.5) +
...
gsi4 <- gsi3 %>% filter(julian_date < 1000) %>%
  mutate(gsiprop = prop*100) %>%
  select(year, julian_date, gsiprop)


bc4 <- bc3 %>%
  mutate(bcprop = prop*100) %>%
  rename(julian_date = revised_julian) %>%
  select(year, julian_date, bcprop)

gsi4$year <- as.factor(gsi4$year)
bc4$year <- as.factor(bc4$year)


dff <- full_join(bc4, gsi4) %>%
  mutate(diff = abs(round(bcprop - gsiprop,2))) %>%
  na.omit() %>%
  group_by(year) %>%
...
View(dff)
ggplot(dff %>% filter(year > "1983"), aes(x=julian_date, y=diff)) +
  geom_bar(stat="identity") +
  facet_wrap(~year)

round(quantile(df$mean,c(0.25,0.5,0.75)),2)
...
ScenA_data$stock <- factor(ScenA_data$stock, levels = c("LowerMainstem","Stewart","Pelly","WhiteDonjek","MiddleMainstem",
                                                                              "Carmacks","UpperLakesAndMainstem","Teslin"))
...
ggplot(ScenA_data, aes(x = as.factor(year), y = proportion*100, fill= stock)) + 
...
ggsave(file.path(figure_path, paste("Figure_7.popn.passage_ScenA_",system_time,".jpg", sep="")),height = 9, width = 7)
...
ScenB_data$population <- factor(ScenB_data$population, levels = c("LowerMainstem","Stewart","Pelly","WhiteDonjek","MiddleMainstem",
                                                                              "Carmacks","UpperLakesandMainstem","Teslin"))
...
ggplot(ScenB_data, aes(x = as.factor(year), y = proportion*100, fill = population)) + 
  ...
  theme(axis.text = element_text(size=10)) +
  theme(axis.title = element_text(size=10))
ggsave(file.path(figure_path, paste("Figure_7.popn.passage_ScenB_",system_time,".jpg", sep="")),height = 9, width = 7)
...
#ggsave(file.path(figure_path, paste("Figure_8.ScenA.product.equilibrium_",system_time,".jpg", sep="")),height = 5, width = 7)
...
ggplot(data = equil_alpha_percentileB, aes(x = equil_med/1000, y = alpha_med, fill=pop)) +
  ...
  theme(axis.text = element_text(size=12)) +
  theme(legend.title=element_blank())
#ggsave(file.path(figure_path, paste("Figure_S2.ScenB.product.equilibrium_",system_time,".jpg", sep="")),height = 5, width = 7)
...
```{r Figure 9 Plot Ricker curves ScenarioA, echo=FALSE, warning=FALSE,message=FALSE, fig.dim=c(6,6)}
...
ggplot() +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(axis.text = element_text(size=10))
#ggsave(file.path(figure_path, paste("Figure_9.Ricker_ScenA_",system_time,".jpg", sep="")),height = 9, width = 7)
...
```{r Figure 9 Plot Ricker curves ScenarioB, echo=FALSE, warning=FALSE,message=FALSE, fig.dim=c(6,6)}


ggplot() +
  geom_ribbon(data = alpha_beta_percentileB, aes(x = abund, ymin = q_20, ymax = q_80), fill = "grey80", alpha=0.5, linetype=2, colour="gray46") +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(axis.text = element_text(size=10))
#ggsave(file.path(figure_path, paste("Figure_9.Ricker_ScenB",system_time,".jpg", sep="")),height = 9, width = 7)
...
ggplot(alpha_resid_df_A, aes(x=year, y = alpha_med + resid_med, color=population), show.legend = F) +
...
ggsave(file.path(figure_path, paste("Figure_10.Prod.index_ScenA_",system_time,".jpg", sep="")),height = 9, width = 7)

```


```{r Figure 10. Productivity index over time - Scenario B, fig.dim=c(8,8)}
...
ggplot(alpha_resid_df_B, aes(x=year, y = alpha_med + resid_med, color=population), show.legend = F) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust = 1, size=10))
ggsave(file.path(figure_path, paste("Figure_10.Prod.index_ScenB_",system_time,".jpg", sep="")),height = 9, width = 7)
...
```{r Figure 11. Usmy posteriors Scenario A, echo=FALSE, fig.dim=c(6,6)}
...
ggplot(Umsy_A, aes(x=substock, y=value, fill=substock)) +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(legend.position="none")
#ggsave(file.path(figure_path, paste("Figure_11.Umsy_ScenA_",system_time,".jpg", sep="")),height = 5, width = 7)
```



```{r Figure 11. Usmy posteriors Scenario B, echo=FALSE, fig.dim=c(6,6)}
# Scenario B
...
ggplot(Umsy_B, aes(x=substock, y=value, fill=substock)) +
  ...
  theme(axis.title = element_text(size=12)) +
  theme(legend.position="none")
#ggsave(file.path(figure_path, paste("Figure_11.Umsy_ScenB_",system_time,".jpg", sep="")),height = 5, width = 7)
...
```{r Figure 12 Portfolio Effects, echo=FALSE}
...
```{r Figure 13 tradeoff plots Scenario A, echo=FALSE, message=FALSE,warning=FALSE, fig.dim=c(6,6)}
# head(t3.3)

# panel a: overfished
ggplot(t3.3, aes(x=U*100, y=med_V2/300)) +
  ggtitle("Overfished") +
  geom_line(linetype = "solid", size=1.25, color="blue") +
  geom_ribbon(aes(ymin=low_V2/300, ymax=upp_V2/300), alpha=0.2, color="blue", lty="dashed", fill = "blue") +
  ...
  scale_y_continuous(sec.axis = sec_axis(~./1, name = "Stocks overfished (%)")) +
...
```{r Extirpation Scenario A Figure 13 tradeoffs, echo=FALSE,message=FALSE,warning=FALSE, fig.dim=c(6,6)}
...
ggplot(t3.3, aes(x=U*100, y=med_V2/300)) +
  ...
  scale_y_continuous(sec.axis = sec_axis(~./1, name = "Stocks exirpated (%)")) +
  scale_x_continuous(limits = c(0,100), breaks=c(0,20,40,60,80,100)) +
  ylab("Harvest (000s)") +
  xlab("Harvest rate (%)") +
  theme_classic() +

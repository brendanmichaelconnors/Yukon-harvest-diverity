...
ggplot() +
  ...
  geom_point(data = Rec_and_Spwn_ScenA, aes(x= Spw_med/1000, y = Rec_med/1000, color=as.factor(year), width=0.9), size=2) +
...
ggplot() +
  ...
  geom_point(data = Rec_and_Spwn_ScenB, aes(x= Spw_med/1000, y = Rec_med/1000, color=as.factor(year), width=0.9), size=2) +
...
ggplot(alpha_resid_df_A, aes(x=year, y = alpha_med + resid_med, fill=population), show.legend = F) +
  geom_point(size=1,show.legend = F, aes(colour = factor(population))) +
  geom_line(show.legend = F, aes(colour = factor(population))) +
  geom_ribbon(aes(ymin=alpha_lwr + resid_lwr, ymax=alpha_upr + resid_upr, colour=factor(population)), show.legend = F, alpha=0.2) +
  scale_fill_viridis(option="viridis", discrete = TRUE) +
